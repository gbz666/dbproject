...
搜索词：[最佳搜索关键词]"""
    ...
    response = llm.invoke([SystemMessage(content="你是一个意图分析助手。"),HumanMessage(content=understand_prompt)])
...
请提供一个有用的回答，并说明这是基于已有知识的回答。"""
        ...
        response = llm.invoke([SystemMessage("你是一个总结能手，负责结论落地"),HumanMessage(content=fallback_prompt)])
...
def reflect_node(state: SearchState) -> dict: # 这里的返回会更新 state
    """步骤4：评估回答质量"""
    ...
    if current_count >= 3:
        return {
            "step": "accepted", 
            "count": current_count, 
            "messages": [AIMessage(content="✅ 已达到最大尝试次数，输出当前最佳答案。")]
        }

    reflect_prompt = f"""
    请评估以下回答是否完美解决了用户的问题。
    用户问题：{state['user_query']}
    AI回答：{state['final_answer']}
    
    如果是，请回复：[finish]是
    如果不是，请回复：[finish]否，并简述原因。
    """
    
    response = llm.invoke([HumanMessage(content=reflect_prompt)])
    
    if "[finish]是" in response.content:
        return {
            "step": "accepted", 
            "count": current_count, 
            "messages": [AIMessage(content="✅ 回答通过质量检查。")]
        }
    else:
        return {
            "step": "rejected", 
            "count": current_count, 
            "messages": [AIMessage(content=f"⚠️ 质量评估未通过，正在进行第 {current_count} 次优化...")]
        }

def decide_next_step(state: SearchState):
    # 根据反思节点设置的状态来判断
    if state["step"] == "accepted":
        return END  # 质量达标，结束流程
    else:
        return "understand"  # 质量不合格，回到起点优化关键词
# 构建搜索工作流
...
async def main():
    ...
    # 打印反思结果，看看它是通过了还是被拦截了
                                print(f"🛡️ 质量评估阶段: {latest_message.content}")
    # 如果 count 增加了，也可以打印出来
                                print(f"🔄 当前迭代次数: {node_output.get('count', 0)}")
